...
#   data/br_clean.csv: toy data cleaned up of non-campaign data
#   data/br_field_setups.rds: per-constituent set up needed for downstream scripts to draw fresh MC fields from
#   data/br_synthetic_extended.csv: the static synthetic extended dataset (long format)
#   data/*.csv, plots/*.png: diagnostics for each section
...
## This whole pipeline is scoped to one watershed at a time. To adapt to a different single watershed, change these and the matching entries in 01_build_spatial_data.R's `networks` list.
project          <- "nm"
...
ssn_site_id_col  <- "Site"      # matches the toy dataset directly
ssn_additive_col <- "afvFlow"   # real flow-accumulation AFV, from 01's Part B2
excluded_sites   <- NA  # must match 01_build_spatial_data.R
constituents     <- c("NPOC..mg.C.L.", "TDN..mg.N.L.")
...
# Note: nm's sites can't all be visited in a day, so a campaign is a run of consecutive calendar dates (gap <= max_gap_days). in br, all can be visited in one day, but this double checks that. Isolated dates not part of a run of >= min_run_length are one-off revisits, not full campaigns, and are discarded.
...
write_csv(clean, file.path(data_out_dir, "br_clean.csv"))

#### PART B -- Fit SSN2 spatial stream network covariance model ####

# Read in ssn object
ssn_object <- readRDS(file.path(data_out_dir, "ssn_objects_br.rds"))[[project]]
...
drop_na_response <- function(ssn.object, resp, label) {
  ...
  ## Scratch object, written to tempdir() 
  sub_path <- file.path(tempdir(), paste0(
    tools::file_path_sans_ext(basename(ssn_path)), "_", make.names(label), "_sub.ssn"
  ))
  subset_expr <- bquote(!is.na(.(as.symbol(resp))))
...
varcomp_summary <- bind_rows(varcomp_rows)
write_csv(varcomp_summary, file.path(data_out_dir, "SSN_covariance_summary_br.csv"))
saveRDS(all_fits, file.path(data_out_dir, "SSN_fitted_models_br.rds"))
...
covariance_choice_overrides <- tibble::tribble(
  ~Constituent,    ~Model,
  "NPOC..mg.C.L.", "none",
  "TDN..mg.N.L.",  "euclid_only"
)
# any spatial structure for NPOC is not indicated by model comparison,  also required a much higher than default ridge and produced unrealistically high simulated values (see part D below), so we're going with no spatial structure. For TDN, AIC scores are almost indistinguishable between no spatial structure and euclidean, and euclidean spatial structure has lower RMSPE and is numerically stable, so we're going with euclidean for TDN.
...
saveRDS(field_setups, file.path(data_out_dir, "br_field_setups.rds"))
...
synthetic_extended <- bind_rows(synthetic_rows)
write_csv(synthetic_extended, file.path(data_out_dir, "br_synthetic_extended.csv"))
...
p_synth_check <- synthetic_extended %>%
  ...
  labs(x = "Synthetic year", y = "Concentration (mean across predpts, that month)",
       title = "Static extended dataset")+
  theme_bw()
ggsave(file.path(plot_dir, "Synthetic_extended_mean_by_month_br.png"), p_synth_check, width = 8, height = 6, dpi = 150)
